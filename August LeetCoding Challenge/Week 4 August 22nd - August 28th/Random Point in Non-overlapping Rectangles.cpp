...
public:
    ...
    Solution(vector<vector<int>>& rects) {
        ...
        for(vector<int> r : rects) {
            totalPoints += (r[2] - r[0] + 1)*(r[3] - r[1] + 1);
            
            hash.push_back(totalPoints);
        }
    }
    
    vector<int> pick() {
        int rr = rand()%totalPoints;
        int index = upper_bound(hash.begin(), hash.end(), rr) - hash.begin();
        
        vector<int> result(2);
        result[0] = rect[index][0] + rand()%(rect[index][2] - rect[index][0] + 1);
        result[1] = rect[index][1] + rand()%(rect[index][3] - rect[index][1] + 1);
        return result;
    }
};
