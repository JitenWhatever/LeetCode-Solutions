    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (String cpdomain : cpdomains) {
            ...
            for (int index = 0; index < parts[1].length(); ++index) {
               if (parts[1].charAt(index) == '.') {
                   String domain = parts[1].substring(index + 1);
                   hash.put(domain, hash.getOrDefault(domain, 0) + rep);
               }
            }
            
            hash.put(parts[1], hash.getOrDefault(parts[1], 0) + rep);
        }
        
        List<String> result = new ArrayList<>();
        
        for (String domain : hash.keySet()) {
            result.add(hash.get(domain) + " " + domain);
        }
