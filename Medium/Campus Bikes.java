...
class Solution {
    public int[] assignBikes(int[][] workers, int[][] bikes) {
        ...
        List<int[]>[] hash = new List[MAX];

        for(int i = 0; i < N; ++i) {
            for(int j = 0; j < M; ++j) {
                int distance = Math.abs(workers[i][0] - bikes[j][0]) + Math.abs(workers[i][1] - bikes[j][1])
                if(hash[distance] == null) {
                    hash[distance] = new ArrayList<>();
                }

                hash[distance].add(new int[]{i, j});
            }
        }

        for(int index = 0; index < MAX; ++index) {
            int w = hash[index].get(index)[0];
            int b = hash[index].get(index)[1];

            if(!assigned[w] && !occupied[b]) {
                result[w] = b;

                assigned[w] = occupied[b] = true;
            }
        }
