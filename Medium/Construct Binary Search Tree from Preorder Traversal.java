...
class Solution {
    public TreeNode bstFromPreorder(int[] preorder) {
        ...
        TreeNode root = helper(preorder, Integer.MAX_VALUE);
        
        return root;
    }
    private int currentIndex = 0;
    private TreeNode helper(int[] preorder, int limit) {
        if(currentIndex == preorder.length || preorder[currentIndex] > limit) {
            return null;
        }
