...
class Solution {
    public int totalFruit(int[] tree) {
        ...
        while(right < tree.length) {
            hash.put(tree[right], hash.getOrDefault(tree[right++], 0) +  1);
            
            while(left < tree.length && hash.size() > 2) {
                hash.put(tree[left], hash.getOrDefault(tree[left], 0) - 1);
                if(hash.getOrDefault(tree[left], 0) == 0) {
                    hash.remove(tree[left]);
                }
