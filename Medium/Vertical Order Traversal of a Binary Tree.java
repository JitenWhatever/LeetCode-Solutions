...
class Solution {
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        for(Map.Entry<Integer, List<Pair>> itr :  hash.entrySet()) {
            List<Pair> list =  itr.getValue();
    ...
    private Map<Integer, List<Pair>> hash = new TreeMap<>();
    ...
    private void dfs(TreeNode root, int depth, int position) {
        ...
        if(hash.containsKey(position)) {
            List<Pair> lst = hash.get(position);
            lst.add(new Pair(root.val, depth));
            hash.put(position, lst);
        }
        else {
            hash.put(position, new ArrayList());
            List<Pair> lst = hash.get(position);
            lst.add(new Pair(root.val, depth));
            hash.put(position, lst);
        }
