...
class Solution {
    public List<String> mostVisitedPattern(String[] username, int[] timestamp, String[] website) {
        Map<String, List<Pair>> userToWebMap = new HashMap<>();
        
        for (int i = 0; i < website.length; i++) {
            userToWebMap.putIfAbsent(username[i], new ArrayList<>());
            userToWebMap.get(username[i]).add(new Pair(timestamp[i], website[i]));
        }
        ...
        for (String user : userToWebMap.keySet()) {
            Set<String> seen = new HashSet<>();
            List<Pair> webs = userToWebMap.get(user);
            Collections.sort(webs, (a, b)->(a.time - b.time));
            
            for (int i = 0; i < webs.size(); i++) {
                for (int j = i + 1; j < webs.size(); j++) {
                    for (int k = j + 1; k < webs.size(); k++) {
                        String str = webs.get(i).web + " " + webs.get(j).web + " " + webs.get(k).web;
